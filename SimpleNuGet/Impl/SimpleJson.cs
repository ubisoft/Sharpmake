...
/// </returns>
        ...
        static class SimpleJson
    {
        ...
        private static readonly char[] s_escapeTable;
        private static readonly char[] s_escapeCharacters = new char[] { '"', '\\', '\b', '\f', '\n', '\r', '\t' };
        private static readonly string s_escapeCharactersString = new string(s_escapeCharacters);

        static SimpleJson()
        {
            s_escapeTable = new char[93];
            s_escapeTable['"'] = '"';
            s_escapeTable['\\'] = '\\';
            s_escapeTable['\b'] = 'b';
            s_escapeTable['\f'] = 'f';
        ...
        public static string SerializeObject(object json, IJsonSerializerStrategy jsonSerializerStrategy)
        {
            ...
            bool success = SerializeValue(jsonSerializerStrategy, json, builder, indentLevel: 0);
        ...
        private static bool SerializeValue(IJsonSerializerStrategy jsonSerializerStrategy, object value, StringBuilder builder, int indentLevel)
        {
            bool success = true;
            ...
            else
            {
                ...
                if (dict != null)
                {
                    success = SerializeObject(jsonSerializerStrategy, dict.Keys, dict.Values, builder, indentLevel);
                }
                else
                {
                    ...
                    {
                        success = SerializeObject(jsonSerializerStrategy, stringDictionary.Keys, stringDictionary.Values, builder, indentLevel);
                    }
                    else
                    {
                        IEnumerable enumerableValue = value as IEnumerable;
                        if (enumerableValue != null)
                            success = SerializeArray(jsonSerializerStrategy, enumerableValue, builder, indentLevel);
                        ...
                        else
                        {
                            ...
                            if (success)
                                SerializeValue(jsonSerializerStrategy, serializedObject, builder, indentLevel);
                        }
                    }
                }
            }
        ...
        private static bool SerializeObject(IJsonSerializerStrategy jsonSerializerStrategy, IEnumerable keys, IEnumerable values, StringBuilder builder, int indentLevel)
        {
            builder.Append("{");
            indentLevel++;
            ...
            while (ke.MoveNext() && ve.MoveNext())
            {
                ...
                builder.AppendIndentation(indentLevel);

                string stringKey = key as string;
                if (stringKey != null)
                    SerializeString(stringKey, builder);
                else if (!SerializeValue(jsonSerializerStrategy, value, builder, indentLevel)) return false;
                builder.Append(": ");
                if (!SerializeValue(jsonSerializerStrategy, value, builder, indentLevel))
                    return false;
                first = false;
            }
        ...
        private static bool SerializeArray(IJsonSerializerStrategy jsonSerializerStrategy, IEnumerable anArray, StringBuilder builder, int indentLevel)
        {
            builder.Append("[");
            indentLevel++;
        ...
        private static void AppendIndentation(this StringBuilder builder, int indentLevel)
        {
        ...
        private static bool SerializeString(string aString, StringBuilder builder)
        {
            ...
            if (aString.IndexOfAny(s_escapeCharacters) == -1)
            {
                builder.Append('"');
            ...
            for (int i = 0; i < charArray.Length; i++)
            {
                ...
                if (c >= s_escapeTable.Length || s_escapeTable[c] == default(char))
                {
                    safeCharacterCount++;
                }
                else
                {
                    ...
                    builder.Append('\\');
                    builder.Append(s_escapeTable[c]);
                }
            }
        ...
        public static IJsonSerializerStrategy CurrentJsonSerializerStrategy
        {
            get
            {
                return s_currentJsonSerializerStrategy ??
                       (s_currentJsonSerializerStrategy =
 ...
 PocoJsonSerializerStrategy
            ...
            set { s_currentJsonSerializerStrategy = value; }
        }

        private static PocoJsonSerializerStrategy s_pocoJsonSerializerStrategy;

        [EditorBrowsable(EditorBrowsableState.Advanced)]
        public static PocoJsonSerializerStrategy PocoJsonSerializerStrategy
        {
            get { return s_pocoJsonSerializerStrategy ?? (s_pocoJsonSerializerStrategy = new PocoJsonSerializerStrategy()); }
        }
        ...
        class PocoJsonSerializerStrategy : IJsonSerializerStrategy
    {
        ...
        private static readonly string[] s_iso8601Format = new string[]
        {
            ...
            if (str != null)
            {
                if (str.Length != 0) // We know it can't be null now.
                {
                    if (type == typeof(DateTime) || (ReflectionUtils.IsNullableType(type) && Nullable.GetUnderlyingType(type) == typeof(DateTime)))
                        return DateTime.ParseExact(str, s_iso8601Format, CultureInfo.InvariantCulture,
                            DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal);

                    if (type == typeof(DateTimeOffset) || (ReflectionUtils.IsNullableType(type) && Nullable.GetUnderlyingType(type) == typeof(DateTimeOffset)))
                        return DateTimeOffset.ParseExact(str, s_iso8601Format, CultureInfo.InvariantCulture,
                            DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal);
        ...
        protected virtual bool TrySerializeKnownTypes(object input, out object output)
        {
            ...
            if (input is DateTime)
                output = ((DateTime)input).ToUniversalTime().ToString(s_iso8601Format[0], CultureInfo.InvariantCulture);
            else if (input is DateTimeOffset)
                output = ((DateTimeOffset)input).ToUniversalTime().ToString(s_iso8601Format[0], CultureInfo.InvariantCulture);
    ...
    {
        private static readonly object[] s_emptyObjects = new object[] { };
        ...
        public static GetDelegate GetGetMethodByReflection(PropertyInfo propertyInfo)
        {
            ...
            return delegate (object source) { return methodInfo.Invoke(source, s_emptyObjects); };
        }
